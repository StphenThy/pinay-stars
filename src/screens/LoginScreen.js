  ...
  return (
    <KeyboardAvoidingView style={s.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" bounces>
        <LinearGradient colors={gradients.hero.colors} locations={gradients.hero.locations} style={[s.top, compact && s.topCompact]}>
          <View style={s.topRow}>
            <Pressable onPress={onBack} hitSlop={12} style={s.backButton}>
              <Ionicons name="chevron-back" size={20} color={colors.onDarkSoft} /><Text style={s.backText}>Back</Text>
            </Pressable>
          ...
          <View style={s.card}>
            ...
            <Field label="Password">
              <View style={s.passwordRow}>
                ...
                />
                <Pressable onPress={() => setShow(x => !x)} hitSlop={8} style={s.eye} accessibilityRole="button" accessibilityLabel={show ? 'Hide password' : 'Show password'}>
                  <Ionicons name={show ? 'eye-off-outline' : 'eye-outline'} size={22} color={colors.rose} />
                </Pressable>
              </View>
            </Field>
            {registering ? (
...
const s = StyleSheet.create({
  ...
  top: { paddingTop: 16, paddingBottom: 24, paddingHorizontal: 24, alignItems: 'center', borderBottomLeftRadius: 28, borderBottomRightRadius: 28 },
  topCompact: { paddingBottom: 16 },
  titleCompact: { fontFamily: fonts.sans, fontSize: 22, marginTop: 4 },
  topRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', marginBottom: 16 },
  backButton: { width: 70, paddingVertical: 8 },
  backText: { color: colors.onDarkSoft, fontFamily: fonts.sansBold, fontSize: 16 },
  topKicker: { color: colors.onDarkSoft, fontSize: 11, fontFamily: fonts.sansBold, letterSpacing: 1.8 },
  logo: { width: 88, height: 88, borderRadius: radius.xl, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  logoImage: { width: 88, height: 88 },
  title: { fontFamily: fonts.serif, fontSize: 28, color: colors.onDarkSoft, marginTop: 12, textAlign: 'center' },
  lead: { color: colors.onDarkSoft, fontFamily: fonts.sans, fontSize: 13, lineHeight: 19, marginTop: 8, textAlign: 'center' },
  ...
  eyeText: { color: colors.rose, fontFamily: fonts.sansBold, fontSize: 13 },
  error: { color: colors.danger, fontFamily: fonts.sansSemi, fontSize: 13, marginBottom: 12 },
  switchCard: { marginTop: 16, flexDirection: 'row', alignItems: 'center', backgroundColor: colors.blushDeep, borderRadius: radius.lg, padding: 16 },
  switchTitle: { color: colors.burgundy, fontFamily: fonts.sansBold, fontSize: 16 },
  switchBody: { color: colors.text, fontFamily: fonts.sans, fontSize: 13, marginTop: 4 },
  switchCta: { color: colors.burgundy, fontFamily: fonts.sansBold, fontSize: 15, marginLeft: 12 },
  guestLink: { alignItems: 'center', paddingVertical: 16 },
  guestText: { color: colors.text, fontFamily: fonts.sans, fontSize: 13 },
  guestStrong: { color: colors.burgundy, fontFamily: fonts.sansBold },
});
