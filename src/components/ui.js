...
export function Tag({ label, tone = 'blush' }) {
  ...
  const tones = {
    ...
    dark: { bg: colors.primary, fg: colors.onPrimary },
  };
 ...
 */
function StateCard({ icon, iconColor, title, body, action, onAction, spinner, tone }) {
  const { colors } = useTheme();
  const s = useThemedStyles(makeStyles);
  return (
    <Rise style={[s.state, tone === 'error' && s.stateError]}>
      <View style={[s.stateIcon, tone === 'error' && s.stateIconError]}>
        {spinner ? <ActivityIndicator color={colors.burgundy} /> : <Ionicons name={icon} size={26} color={iconColor || colors.burgundy} />}
      </View>
...
export function Button({ label, onPress, variant = 'primary', disabled, style, small, icon, accessibilityLabel }) {
  ...
  const s = useThemedStyles(makeStyles);
  const variants = {
    primary: [s.btnPrimary, s.btnPrimaryText, colors.onPrimary],
    secondary: [s.btnSecondary, s.btnSecondaryText, colors.burgundy],
    ghost: [s.btnGhost, s.btnGhostText, colors.text],
    danger: [s.btnDanger, s.btnDangerText, colors.onDanger],
  };
...
export function IconButton({ icon, onPress, label, size = 20, color, style, badge, tone = 'blush' }) {
  const { colors } = useTheme();
  const s = useThemedStyles(makeStyles);
  return (
    <Pressable
    ...
    >
      <Ionicons name={icon} size={size} color={color || colors.burgundy} />
...
const makeStyles = ({ colors, type }) => StyleSheet.create({
  ...
  chip: { flexDirection: 'row', alignItems: 'center', minHeight: touch.min, backgroundColor: colors.surface, borderRadius: radius.pill, paddingVertical: space.sm, paddingHorizontal: space.lg, borderWidth: 1, borderColor: colors.line },
  ...
  chipCountActive: { color: colors.primary, backgroundColor: colors.white },
  ...
  state: { marginHorizontal: space.page, marginVertical: space.lg, padding: space.xxl, alignItems: 'center', backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.line },
  stateError: { borderColor: colors.dangerSoft, backgroundColor: colors.surfaceAlert },
  ...
  btnDanger: { backgroundColor: colors.dangerFill },
  btnDangerText: { color: colors.onDanger },
  ...
  iconBadge: { position: 'absolute', top: -2, right: -2, minWidth: 18, height: 18, borderRadius: 9, backgroundColor: colors.dangerFill, alignItems: 'center', justifyContent: 'center', paddingHorizontal: space.xs, borderWidth: 2, borderColor: colors.background },
  iconBadgeText: { ...type.micro, color: colors.onDanger },
});
